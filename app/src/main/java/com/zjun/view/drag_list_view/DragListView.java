...
import com.qm.homework.testdemo.Image;
...
public class DragListView extends ListView {
    ...
    private OnItemChangeListening mItemChangeListening;
    /**
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                // 如果触摸的坐标不在条目上，在分割线、或外部区域，则为无效值-1; 宽度1/4 以右的区域可拖拽; Header和Footer无效
                if (!isPositionValid(mDragPosition) || mDownX < getWidth() * 1 / 4) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                recycleVelocityTracker();
                if (mItemChangeListening != null) {
                    mItemChangeListening.onItemChange(index);
                }
                break;
            default:
                break;
...
//            mAdapter.swapData(mFromPosition, mToPosition);
            ...
            index = new int[getChildCount()];
            for (int i = 0; i < getChildCount(); i++) {
                if (i==0){
                    index[0]=0;
                }else {
                    Image item = (Image) getAdapter().getItem(i);
                    index[i] = item.getTag();
                }
            }
            return true;
    ...
    public interface OnItemChangeListening{
        void onItemChange(int[] index);
    }

    public void setmItemChangeListening(OnItemChangeListening mItemChangeListening) {
        this.mItemChangeListening = mItemChangeListening;
    }
}
